...
_exports.train = speller.train = function (text) {
	var word,
	    m,
	    r = /[a-z]+/g;
...
_exports.correct = speller.correct = function (word) {
...
_exports.nWords = speller.nWords = {};
...
if (typeof module !== 'undefined' && module.exports) {
	var fs = require('fs');
	var start = new Date();
	fs.readFile(__dirname + '/big.txt', 'ascii', function (err, data) {
		if (err) throw err;
		// Train the language model first.
		speller.train(data);
		var t1 = new Date();
		console.log('Trained in ' + (t1 - start) + ' msec');
		// Then correct the provided word.
		console.log(speller.correct("spume"));
		var finish = new Date();
		console.log('Found result in ' + (finish - t1) + ' msec');
	});
} else {
